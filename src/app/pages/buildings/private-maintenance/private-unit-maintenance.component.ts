...
@Component({
  selector: 'app-private-unit-maintenance',
  standalone: true,
  imports: [
    CommonModule,
    DatePipe,
      ...
      <div class="mb-4 flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-slate-600">
            {{ i18n.translate('maintenance.private.subtitle') }}
          </p>
        </div>
      ...
      @if (showCreate()) {
        <mat-card class="mb-6 p-4">
          <h2 class="mb-4 text-lg font-semibold">
            {{ i18n.translate('maintenance.private.createTitle') }}
          </h2>
          <form [formGroup]="form" (ngSubmit)="submit()" class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div class="md:col-span-2 flex justify-end">
              <button mat-flat-button color="primary" type="submit" [disabled]="form.invalid || loading()">
                {{ i18n.translate('maintenance.private.create') }}
              </button>
            </div>
          </form>
        </mat-card>
      ...
      <mat-card class="p-4">
        <div class="mb-3 flex items-center justify-between">
          <h2 class="text-lg font-semibold">
            {{ i18n.translate('maintenance.history.privateTitle') }} ({{ totalElements() }})
          </h2>
...
export class PrivateUnitMaintenanceComponent implements OnInit {
  ...
  readonly displayedColumns = ['title', 'status', 'createdAt'];

  readonly showCreate = computed(() => this.mode() === 'create');
  readonly titleKey = computed(() =>
    this.mode() === 'create' ? 'maintenance.private.title' : 'maintenance.history.privateTitle'
  );
  ...
  ngOnInit(): void {
    ...
    this.buildingId.set(
      Number(params.get('id') ?? params.get('buildingId') ?? parentParams?.get('id') ?? parentParams?.get('buildingId') ?? 0)
    );
    this.unitId.set(Number(params.get('unitId') ?? parentParams?.get('unitId') ?? 0));
  ...
  loadList(): void {
    ...
    this.service.listPrivateUnitMaintenance(this.unitId(), this.pageIndex(), this.pageSize()).subscribe({
      next: (page) => {
  ...
  submit(): void {
    ...
    this.loading.set(true);
    this.service
      .createPrivateUnitMaintenance(this.unitId(), {
        title: value.title,
