...
import { BuildingUnit, BuildingUnitDetails, BuildingUnitRequest, InvoiceStatus, UnitAssignee, UnitStatus } from '../../../models/building/building-unit.model';
  ...
  template: `
    <div class="mx-auto max-w-6xl px-4 py-6">
      <div class="mb-4 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-slate-900">Building Units</h1>
          <p class="text-sm text-slate-600">Manage residential and commercial units for this building.</p>
        </div>
      ...
      <mat-card class="mb-6 p-4">
        ...
        <h2 class="mb-4 text-lg font-semibold">{{ editingUnitId() ? 'Edit Unit' : 'Create Unit' }}</h2>
        @if (canManageUnits()) {
        <form [formGroup]="unitForm" (ngSubmit)="submitUnit()" class="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <mat-form-field appearance="outline">
            ...
            <input matInput formControlName="unitType" placeholder="RESIDENTIAL" />
          </mat-form-field>
          ...
          <mat-form-field appearance="outline">
            <mat-label>Monthly fee override</mat-label>
            <input matInput type="number" step="0.01" formControlName="monthlyFeeOverride" />
            <mat-hint>Overrides the building default fee for this unit.</mat-hint>
          </mat-form-field>
          ...
          <mat-slide-toggle formControlName="occupied" class="self-center">Occupied</mat-slide-toggle>

          @if (owners().length > 0) {
            <mat-form-field appearance="outline" class="md:col-span-2">
              <mat-label>Owner (existing user)</mat-label>
              ...
              <mat-hint>Select an existing owner or fill in manual owner details.</mat-hint>
            </mat-form-field>
          }

          <mat-form-field appearance="outline" class="md:col-span-2">
            <mat-label>Owner Name</mat-label>
            <input matInput formControlName="ownerName" placeholder="Optional" />
          </mat-form-field>
          ...
          @if (tenants().length > 0) {
            <mat-form-field appearance="outline" class="md:col-span-2">
              <mat-label>Tenant (existing user)</mat-label>
              ...
              <mat-hint>Select an existing tenant or fill in manual tenant details.</mat-hint>
            </mat-form-field>
          }

          <mat-form-field appearance="outline" class="md:col-span-2">
            <mat-label>Tenant Name</mat-label>
            <input matInput formControlName="tenantName" placeholder="Optional" />
          </mat-form-field>
          ...
          @if (ownersLoading()) {
            <p class="md:col-span-4 text-xs text-slate-500">Loading owner options...</p>
          }
      ...
      <mat-card class="p-4">
        <div class="mb-3 flex items-center justify-between">
          <h2 class="text-lg font-semibold">Units ({{ totalElements() }})</h2>
          <button mat-stroked-button (click)="loadUnits()" [disabled]="loading()">Refresh</button>
        </div>
        ...
        @if (loading()) {
          <p class="text-sm text-slate-500">Loading units...</p>
        }
        ...
        @if (!loading()) {
          <div class="overflow-x-auto">
            <table mat-table [dataSource]="units()" class="w-full">
              ...
              <ng-container matColumnDef="occupied">
                ...
                <td mat-cell *matCellDef="let row">{{ row.occupied ? 'Yes' : 'No' }}</td>
              </ng-container>

              <ng-container matColumnDef="actions">
                <th mat-header-cell *matHeaderCellDef>Actions</th>
                <td mat-cell *matCellDef="let row">
                  <button mat-icon-button color="primary" (click)="startEdit(row)" aria-label="Edit unit" [disabled]="!canManageUnits()">
                    <mat-icon>edit</mat-icon>
                  </button>
                  <a mat-icon-button [routerLink]="['/property', buildingId(), 'units', row.id]" aria-label="Open unit dashboard">
                    <mat-icon>open_in_new</mat-icon>
                  </a>
                  <button mat-icon-button (click)="openUnitDetails(row)" aria-label="Quick unit preview">
          ...
          @if (selectedUnitDetails()) {
            <div class="mt-6 rounded-xl border border-slate-200 bg-gradient-to-br from-white to-slate-50 p-5 shadow-sm">
              @if (detailsLoading()) {
                <p class="text-sm text-slate-500">Loading unit details...</p>
              } @else {
                @if (detailsError()) {
                ...
                @if (selectedUnitDetails(); as details) {
                  <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <h3 class="text-xl font-semibold text-slate-900">Unit {{ details.unitNumber }} Insights</h3>
                      <p class="text-sm text-slate-600">Complete payment history and apartment profile for this unit.</p>
                    </div>
                  ...
                  <div class="mb-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div class="rounded-lg border border-slate-200 bg-white p-4">
                      <h4 class="mb-2 text-sm font-semibold text-slate-700">Owner</h4>
                      <p class="text-sm text-slate-900">{{ details.ownerName || 'Not assigned' }}</p>
                      @if (details.ownerEmail) {
                        <p class="text-xs text-slate-500">{{ details.ownerEmail }}</p>
                      }
                    </div>
                    <div class="rounded-lg border border-slate-200 bg-white p-4">
                      <h4 class="mb-2 text-sm font-semibold text-slate-700">Tenant</h4>
                      <p class="text-sm text-slate-900">{{ details.tenantName || 'Not assigned' }}</p>
                  ...
                  <div>
                    <h4 class="mb-3 text-sm font-semibold text-slate-700">Payment Timeline</h4>
                    @if (details.paymentHistory.length === 0) {
                      <p class="rounded border border-slate-200 bg-white px-3 py-2 text-sm text-slate-500">No payment history linked to current owner/tenant users.</p>
                    } @else {
                      <div class="space-y-2">
                          ...
                          <div class="rounded-lg border border-slate-200 bg-white p-3">
                            <div class="flex flex-wrap items-center justify-between gap-2">
                              <div>
                                ...
                                <p class="text-xs text-slate-500">Due {{ item.dueDate }} · {{ item.payerName || item.payerEmail || 'Unknown payer' }}</p>
                              </div>
