...
export class PropertyDashboardService {
  ...
  getPropertyDashboardVm(id: number): Observable<PropertyDashboardVm> {
    ...
    return this.propertyService.getPropertyById(id).pipe(
      switchMap((property) => {
        const financeSummary$ = canManageBuildingOperations(property.currentUserRole)
          ? this.financeService.getSummary(id).pipe(
              catchError(() => {
                fallbackSources.push('building-finance-summary');
                return of(this.emptyFinanceSummary(id));
              })
            )
          : of(this.emptyFinanceSummary(id));

        return forkJoin({
          financeSummary: financeSummary$,
          linkedUsers: this.propertyService.getPropertyLinkedUsers(id).pipe(
            catchError(() => {
          ...
          map(({ financeSummary, linkedUsers, unitsPage, documentsPage, incidentsPage, workOrdersPage, compliancePage }) => {
            ...
            return {
              id: property.id,
              isBuilding: Boolean(property.isBuilding),
  ...
  private emptyFinanceSummary(buildingId: number): BuildingFinanceSummary {
    return {
      buildingId,
      totalInvoices: 0,
      pendingInvoices: 0,
